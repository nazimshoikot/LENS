...
from utils.model import LeNet5, LeNet5CE
...
# usese read_dataset to load the datasets
def load_dataset(test_image_path, test_label_path, train_image_path, train_label_path):
    trainset = (train_image_path, train_label_path) # tuple of two np arrays
    ...
    # train_image_normalized_pad = train_image_normalized_pad[:5000]
    # test_image_normalized_pad = test_image_normalized_pad[:2000]
...
def train(model, train_data, test_data, num_epoch, lr_global_list, batch_size):
    ...
    cost_last, count = np.Inf, 0    # initilize cost, count and err_list
    err_rate_list = []

    # for every epoch
    for epoch in range(0, num_epoch):
        print("---------- epoch", epoch + 1, "begin ----------")
        lr_global = lr_global_list[epoch]   # find learning rate for this epoch
        ...
        ste = time.time()   # find time before iteration
        cost = 0

        # get random shuffled minibatches - 235 tuples in array
        # args: train image array, train label array, batch size
        ...
        for i in tqdm(range(len(mini_batches))):
            ...
            # loss, dout = model.Forward_Propagation(batch_image, batch_label, 'train')

            cost += loss
            # For your implementation
            model.Back_Propagation(lr_global)
            # model.Back_Propagation(dout,lr_global)
...
def main():
    ...
    x = np.arange(args.epoch) #args.epoch
    plt.xlabel('epoches')
    plt.ylabel('error rate')
    # print("Type of x: {}, type of err_rate_list: {}".format(type(x), type(err_rate_list[0])))
